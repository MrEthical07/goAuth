...
This file belongs to package `session` and implements part of the goAuth authentication stack. It exists to encapsulate the concerns represented by `model.go` and keep hot-path auth logic modular.

## 2) Exported API in This File
- `Session`: public API element defined in this file; see source doc comments for exact behavior, error conditions, and guarantees.
