# `permission/mask_codec.go`

## 1) File Responsibility
This file belongs to package `permission` and implements part of the goAuth authentication stack. It exists to encapsulate the concerns represented by `mask_codec.go` and keep hot-path auth logic modular.

## 2) Exported API in This File
- `EncodeMask`: public API element defined in this file; see source doc comments for exact behavior, error conditions, and guarantees.
- `DecodeMask`: public API element defined in this file; see source doc comments for exact behavior, error conditions, and guarantees.

## 3) Internal Interactions
- Depends on imports such as: bytes, encoding/binary, errors.
