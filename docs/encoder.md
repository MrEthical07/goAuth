...
This file belongs to package `session` and implements part of the goAuth authentication stack. It exists to encapsulate the concerns represented by `encoder.go` and keep hot-path auth logic modular.

## 2) Exported API in This File
- `Encode`: public API element defined in this file; see source doc comments for exact behavior, error conditions, and guarantees.
- `Decode`: public API element defined in this file; see source doc comments for exact behavior, error conditions, and guarantees.

## 3) Internal Interactions
- Depends on imports such as: bytes, encoding/binary, errors, io, github.com/MrEthical07/goAuth/permission.
