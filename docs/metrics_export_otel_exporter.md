# `metrics/export/otel/exporter.go`

## 1) File Responsibility
This file belongs to package `otel` and implements part of the goAuth authentication stack. It exists to encapsulate the concerns represented by `exporter.go` and keep hot-path auth logic modular.

## 2) Exported API in This File
- `OTelExporter`: public API element defined in this file; see source doc comments for exact behavior, error conditions, and guarantees.
- `NewOTelExporter`: public API element defined in this file; see source doc comments for exact behavior, error conditions, and guarantees.
- `NewOTelExporterFromSource`: public API element defined in this file; see source doc comments for exact behavior, error conditions, and guarantees.

## 3) Internal Interactions
- Depends on imports such as: context, errors, fmt, github.com/MrEthical07/goAuth/metrics/export/internaldefs, go.opentelemetry.io/otel/metric, goauth_audit_dropped_total.
