...
This file belongs to package `rate` and implements part of the goAuth authentication stack. It exists to encapsulate the concerns represented by `limiter.go` and keep hot-path auth logic modular.
...
- `Limiter`: public API element defined in this file; see source doc comments for exact behavior, error conditions, and guarantees.
...
- Depends on imports such as: context, errors, fmt, time, github.com/redis/go-redis/v9.
