...
This file belongs to package `middleware` and implements part of the goAuth authentication stack. It exists to encapsulate the concerns represented by `guard.go` and keep hot-path auth logic modular.

## 2) Exported API in This File
- `AuthResultFromContext`: public API element defined in this file; see source doc comments for exact behavior, error conditions, and guarantees.
- `Guard`: public API element defined in this file; see source doc comments for exact behavior, error conditions, and guarantees.

## 3) Internal Interactions
- Depends on imports such as: context, net/http, strings.
