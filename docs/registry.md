# `permission/registry.go`

## 1) File Responsibility
This file belongs to package `permission` and implements part of the goAuth authentication stack. It exists to encapsulate the concerns represented by `registry.go` and keep hot-path auth logic modular.

## 2) Exported API in This File
- `Registry`: public API element defined in this file; see source doc comments for exact behavior, error conditions, and guarantees.
- `NewRegistry`: public API element defined in this file; see source doc comments for exact behavior, error conditions, and guarantees.
