# `middleware/strict.go`

## 1) File Responsibility
This file belongs to package `middleware` and implements part of the goAuth authentication stack. It exists to encapsulate the concerns represented by `strict.go` and keep hot-path auth logic modular.

## 2) Exported API in This File
- `RequireStrict`: public API element defined in this file; see source doc comments for exact behavior, error conditions, and guarantees.
